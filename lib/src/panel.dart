...
class _SlidingUpPanelState extends State<SlidingUpPanel>
  ...
  void initState() {
    ...
    _sc = new DelegatingScrollController(widget.scrollViewCount,
        defaultScrollView: widget.defaultScrollView);
  ...
  void _onGestureEnd(Velocity v) {
    ...
    if (v.pixelsPerSecond.dy.abs() >= minFlingVelocity) {
      ...
      if (widget.panelSnapping && widget.snapPoint != null) {
        ...
        else if ((minDistance == d2Open && v.pixelsPerSecond.dy > 0) ||
            (minDistance == d2Close && v.pixelsPerSecond.dy < 0))
...
class DelegatingScrollController implements ScrollController {
  ...
  ScrollController get _currentScrollController =>
      _currentDelegate.scrollController;
  ...
  DelegatingScrollController(int scrollViewCount, {int defaultScrollView = 0})
      : _delegates = [
          for (int i = 0; i < scrollViewCount; i++)
            _ScrollDelegate(ScrollController())
        ] {
  ...
  void delegateTo(int i) {
    _listeners.forEach(
        (listener) => _currentScrollController.removeListener(listener));
    this._currentDelegate = _delegates[i];
    _listeners
        .forEach((listener) => _currentScrollController.addListener(listener));
  }
  ...
  ScrollPosition createScrollPosition(ScrollPhysics physics,
    ...
    return _currentScrollController.createScrollPosition(
        physics, context, oldPosition);
  }
  ...
  Future<Function> animateTo(double offset,
    ...
    return _currentScrollController.animateTo(offset,
        duration: duration, curve: curve);
  }
  ...
  ScrollController delegate(int i) => _delegates[i].scrollController;
}
